...
function createSetupStore($id, setup, options = {}, pinia, hot$1, isOptionsStore) {
    ...
    const partialStore = {
        ...
        $subscribe(callback, options$1 = {}) {
            const removeSubscription = addSubscription(subscriptions, callback, options$1.detached, () =>
                stopWatcher()
            );
            const stopWatcher = scope.run(() =>
                watch(
                    ...
                    (state) => {
                        if (options$1.flush === 'sync' ? isSyncListening : isListening)
                            callback(
                                {
                                    storeId: $id,
...
function createStaticRpcClient() {
    async function getModuleTransformInfo(query, id) {
        const { hash } = await __vitePreload(
            async () => {
...
const usePayloadStore = defineStore('payload', () => {
    ...
    async function init() {
        ...
        if (
            !metadata.value.instances.some(
                (i) => i.vite === query.value.vite && i.environments.includes(query.value.env)
            )
        ) {
