import { computed } from './runtime-core.esm-bundler-BYdEh_Lp.js';
import { defineStore, usePayloadStore } from './payload-BKj06nBe.js';
...
function get(obj, path) {
    ...
    const deepGet = (obj$1, path$1, index) => {
        ...
        if (!path$1[index]) list.push(obj$1);
            ...
            if (index === path$1.length - 1 && (isString(value) || isNumber(value) || isBoolean(value)))
...
function norm(weight = 1, mantissa = 3) {
    ...
    return {
        get(value) {
            ...
            const norm$1 = 1 / Math.pow(numTokens, 0.5 * weight);
...
function parseQuery(pattern, options = {}) {
    return pattern.split(OR_TOKEN).map((item) => {
        let query = item
            .trim()
            .split(SPACE_RE)
            .filter((item$1) => item$1 && !!item$1.trim());
        let results = [];
...
const convertToExplicit = (query) => ({
    [LogicalOperator.AND]: Object.keys(query).map((key) => ({ [key]: query[key] })),
});
    ...
    results.forEach((result) => {
        ...
        result.matches.forEach(({ key, norm: norm$1, score }) => {
            ...
            totalScore *= Math.pow(
                ...
                (weight || 1) * (ignoreFieldNorm ? 1 : norm$1)
            );
        });
...
var Fuse = class {
    ...
    _findMatches({ key, value, searcher }) {
        ...
        let matches = [];
        if (isArray(value))
            value.forEach(({ v: text, i: idx, n: norm$1 }) => {
                if (!isDefined(text)) return;
                const { isMatch, score, indices } = searcher.searchIn(text);
                if (isMatch)
                    matches.push({
